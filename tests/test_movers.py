...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"min_shift": 20, "max_shift": 10},
            ValueError,
            "min_shift must be less than max_shift",
        ),
        (
            {"min_time": 3, "max_time": 2},
            ValueError,
...
@pytest.mark.parametrize(
    ("current_position", "end_position", "random_shift", "expected_position"),
    [
        (Point(10, 20), Point(20, 30), 6, Point(3, 3)),
        (Point(10, 10), Point(7, 4), 100, Point(-3, -6)),
    ],
)
def test_default_move_generator_get_random_position(
    monkeypatch,
    current_position,
    end_position,
    random_shift,
    ...
    monkeypatch.setattr(
        math_antirobot.random,
        "uniform",
        lambda min_shift, max_shift: random_shift,
    )
    generator = DefaultMoveGenerator(min_shift=0, max_shift=100)

    assert (
        generator._DefaultMoveGenerator__get_random_position(
            current_position,
            end_position,
        )
        == expected_position
...
def test_default_move_generator_call_returns_end_position_if_it_is_close(monkeypatch):
    ...
    monkeypatch.setattr(
        generator,
        "_DefaultMoveGenerator__get_random_time",
        lambda: 0.5,
    )
...
def test_default_move_generator_call_returns_intermediate_and_end_positions(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        generator,
        "_DefaultMoveGenerator__get_random_position",
        fake_get_random_position,
    )

    result = list(generator(Point(0, 0), Point(10, 0)))

    assert positions == [(Point(0, 0), Point(10, 0))]
    assert result == [(Point(5, 0), 0.1), (Point(10, 0), 0.2)]
...
def test_anti_robot_mover_move(monkeypatch):
    ...
    assert moved_points == [(5, 6, 0.1), (7, 8, 0.2)]
